...
export interface ChatbotNavMainProps {
  items: NavMainItemType[];
  chatbotId: string;
}

export function NavMain({ items, chatbotId }: ChatbotNavMainProps) {
      ...
      <SidebarMenu>
        {items.map((item) => {
          const isParentItemActive = 
            pathname === item.url || 
            (item.url && pathname.startsWith(item.url) && (pathname.length === item.url.length || pathname[item.url.length] === '/')) ||
          ...
          if (item.items && item.items.length > 0) {
            return (
              <Collapsible
                key={item.title}
                asChild
                defaultOpen={isParentItemActive}
                className="group/collapsible"
              >
                <SidebarMenuItem className={cn(isParentItemActive && !item.items.some(si => pathname.startsWith(si.url)) && "bg-sidebar-accent text-sidebar-accent-foreground")}>
                  <CollapsibleTrigger asChild>
                    <SidebarMenuButton tooltip={item.title} className={cn("w-full", isParentItemActive && "bg-sidebar-accent text-sidebar-accent-foreground")}>
            ...
            const isActive = pathname === item.url || (item.url && pathname.startsWith(item.url) && (pathname.length === item.url.length || pathname[item.url.length] === '/'));
            return (
