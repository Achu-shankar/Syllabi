...
import { GOOGLE_DRIVE_SKILL_DEFINITIONS } from '../src/services/skills/builtin/google_drive';
...
/**
 * Script to seed built-in Google Drive skills to the database
 * Run with: npx tsx scripts/seed-google-drive-builtin-skills.ts
 */
async function seedGoogleDriveBuiltinSkills() {
  const supabase = createScriptServiceClient();
  
  console.log('🚀 Starting Google Drive built-in skills seeding...');
  console.log(`📦 Found ${GOOGLE_DRIVE_SKILL_DEFINITIONS.length} skill(s) to seed`);

  try {
    for (const skill of GOOGLE_DRIVE_SKILL_DEFINITIONS) {
    ...
    console.log('🎉 Google Drive built-in skills seeding completed successfully!');
    console.log('\n📋 Next steps:');
