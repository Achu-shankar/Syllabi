...
import { GOOGLE_CALENDAR_SKILL_DEFINITIONS } from '../src/services/skills/builtin/google_calendar';
...
async function seedGoogleCalendarBuiltinSkills() {
  const supabase = createScriptServiceClient();
  console.log('🚀 Starting Google Calendar built-in skills seeding...');
  console.log(`📦 Found ${GOOGLE_CALENDAR_SKILL_DEFINITIONS.length} skill(s) to seed`);
  try {
    for (const skill of GOOGLE_CALENDAR_SKILL_DEFINITIONS) {
    ...
    console.log('🎉 Google Calendar built-in skills seeding completed successfully!');
    console.log('\n📋 Next steps:');
...
if (require.main === module) {
  seedGoogleCalendarBuiltinSkills()
    .then(() => {
