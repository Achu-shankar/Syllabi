...
import { SLACK_SKILL_DEFINITIONS } from '../src/services/skills/builtin/slack';
...
/**
 * Script to seed built-in Slack skills to the database
 * Run with: npx tsx scripts/seed-slack-builtin-skills.ts
 */
async function seedSlackBuiltinSkills() {
  const supabase = createScriptServiceClient();
  
  console.log('🚀 Starting Slack built-in skills seeding...');
  console.log(`📦 Found ${SLACK_SKILL_DEFINITIONS.length} skill(s) to seed`);

  try {
    for (const skill of SLACK_SKILL_DEFINITIONS) {
    ...
    console.log('🎉 Slack built-in skills seeding completed successfully!');
    console.log('\n📋 Next steps:');
    console.log('1. Users can now see these skills in their skills page');
    console.log('2. To use them, users need to connect their Slack integration');
    console.log('3. The skills will automatically appear when Slack is connected');
